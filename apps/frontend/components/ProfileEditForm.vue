<template>
  <div style="margin-top: 2rem; margin-bottom: 2rem;">
    <div style="background: var(--color-bg-secondary); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border-radius: 1rem; padding: 2rem; border: 1px solid var(--color-border-tertiary);">
      ...
      <form @submit.prevent="$emit('save')" style="display: flex; flex-direction: column; gap: 1.5rem;">
        ...
        <div style="display: grid; grid-template-columns: 1fr; gap: 1.5rem;">
          <!-- Header Background -->
          <div style="display: flex; flex-direction: column; gap: 0.5rem;">
            <label style="display: block; font-size: 0.875rem; font-weight: 600; color: var(--color-text-secondary);">{{ $t('profile.backgroundImage') }} (Header)</label>
            <div style="display: flex; gap: 1rem; align-items: center;">
          ...
          <div style="display: flex; flex-direction: column; gap: 0.5rem;">
            <label style="display: block; font-size: 0.875rem; font-weight: 600; color: var(--color-text-secondary);">Global Background (Full Page)</label>
            <div style="display: flex; gap: 1rem; align-items: center;">
              <div style="width: 6rem; height: 3rem; border-radius: 0.5rem; overflow: hidden; border: 2px solid var(--color-border-primary);">
                <div
                    v-if="isBase64Image(editData.globalBackground)"
                    style="width: 100%; height: 100%; background-size: cover; background-position: center; background-repeat: no-repeat;"
                    :style="{ backgroundImage: `url(${editData.globalBackground})` }"
                ></div>
                <div
                    v-else
                    style="width: 100%; height: 100%; background: linear-gradient(135deg, #e0c3fc 0%, #8ec5fc 100%); display: flex; align-items: center; justify-content: center; font-size: 0.75rem; color: #333;"
                >
                  Gradient
                </div>
              </div>
              <div style="flex: 1;">
                <div style="position: relative;">
                  <input
                      ...
                      accept="image/*"
                      style="position: absolute; opacity: 0; width: 0; height: 0;"
                      @change="handleGlobalBackgroundUpload"
                  />
                  <button
                      type="button"
                      @click="triggerGlobalBackgroundInput"
                      style="width: 100%; padding: 0.5rem 1rem; background: var(--color-bg-tertiary); border: 1px solid var(--color-border-secondary); border-radius: 0.5rem; cursor: pointer; transition: all 0.2s; font-size: 0.875rem; color: var(--color-text-secondary);"
                      onmouseover="this.style.backgroundColor='var(--color-border-primary)'"
                      onmouseout="this.style.backgroundColor='var(--color-bg-tertiary)'"
                  >
                ...
                <button
                    v-if="editData.globalBackground"
                    type="button"
                    @click="$emit('update:globalBackground', '')"
                    style="width: 100%; margin-top: 0.5rem; padding: 0.25rem 0.5rem; background: var(--color-danger-bg); border: 1px solid var(--color-danger-light); border-radius: 0.375rem; cursor: pointer; transition: all 0.2s; font-size: 0.75rem; color: var(--color-danger);"
                    onmouseover="this.style.backgroundColor='var(--color-danger-light)'"
                    onmouseout="this.style.backgroundColor='var(--color-danger-bg)'"
                >
...
const emit = defineEmits(['save', 'cancel', 'update:name', 'update:pronouns', 'update:avatar', 'update:bio', 'update:background', 'update:globalBackground', 'update:location', 'update:website', 'update:currentCompany', 'update:currentCompanyLink', 'update:currentSchool', 'update:currentSchoolLink', 'avatar-upload', 'background-upload', 'export-data', 'import-data'])

const fileInput = ref(null)
const backgroundInput = ref(null)
const globalBackgroundInput = ref(null)
...
const triggerGlobalBackgroundInput = () => {
  if (globalBackgroundInput.value) {
...
const handleGlobalBackgroundUpload = (event) => {
  const file = event.target.files[0]
  if (!file) return
  ...
  const reader = new FileReader()
  reader.onload = (e) => {
    emit('update:globalBackground', e.target.result)
  }
  reader.onerror = (e) => {
    console.error('File reading error:', e)
    showNotification('error', t('common.tips'), t('profile.uploadError'), e.message || String(e))
